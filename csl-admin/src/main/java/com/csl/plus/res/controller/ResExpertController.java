...
public class ResExpertController {
    ...
    /**
     ...
     @GetMapping("/info/{id}")
     @PreAuthorize("hasAuthority('res:resexpert:info')") public R info(@PathVariable("id") Long id){
    ...
    public Object updateVerifyStatus(@RequestParam("ids") Long ids,
        ...
        int count = resExpertService.updateVerifyStatus(ids, verifyStatus, detail);
        if (count > 0) {
    ...
    public Object fetchVList(@PathVariable Long id, @PathVariable String sysGroup) {
        List<ReviewLog> list = resExpertService.getVertifyRecord(id, sysGroup);
