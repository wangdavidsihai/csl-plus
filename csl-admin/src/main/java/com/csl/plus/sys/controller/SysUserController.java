...
import com.csl.plus.sys.service.ISysPermissionService;
...
/**
 * <p>
 * 后台用户表 前端控制器,主要用于后台管理人员
 * </p>
 *
 * @since 2019-04-14
 */
@Slf4j
@Api(value = "系统用户管理", description = "", tags = {"系统用户管理"})
@RestController
...
public class SysUserController extends ApiController {
    ...
    public Object getUserByPage(SysUser entity, @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize) {
        try {
            return new CommonResult()
                    .success(sysUserService.page(new Page<SysUser>(pageNum, pageSize), new QueryWrapper<>(entity)));
        } catch (Exception e) {
    ...
    public Object getAdminInfo(Principal principal) {
        ...
        SysUser umsAdmin = sysUserService.getOneUserInfo(queryU);
        Map<String, Object> data = new HashMap<>();
        data.put("username", umsAdmin.getUsername());
        data.put("roles", umsAdmin.getRoleIds().split(","));
    ...
    @ResponseBody
    public Object updateRole(@RequestParam("adminId") Long adminId, @RequestParam("roleIds") List<Long> roleIds) {
     ...
     * 修改
     */
    @SysLog(MODULE = "sys", REMARK = "修改用户状态")
    @ApiOperation("修改用户状态")
