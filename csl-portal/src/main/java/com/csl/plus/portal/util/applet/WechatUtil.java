package com.csl.plus.portal.util.applet;
...
import com.csl.plus.portal.util.MapUtils;
import com.csl.plus.portal.util.XmlUtil;
...
public class WechatUtil {
	...
	/**
	 * 方法描述：微信退款逻辑 创建时间：2017年4月12日 上午11:04:25 作者： xubo
	 *
	 * @param
	 * @return
	 */
	/*
	 * public static WechatRefundApiResult wxRefund(String out_trade_no, Double
	 * orderMoney, Double refundMoney) { //初始化请求微信服务器的配置信息包括appid密钥等 //转换金钱格式
	 * BigDecimal bdOrderMoney = new BigDecimal(orderMoney, MathContext.DECIMAL32);
	 * BigDecimal bdRefundMoney = new BigDecimal(refundMoney,
	 * MathContext.DECIMAL32); //构建请求参数 Map<Object, Object> params =
	 * buildRequsetMapParam(out_trade_no, bdOrderMoney, bdRefundMoney); String
	 * mapToXml = MapUtils.convertMap2Xml(params); //请求微信 String reponseXml =
	 * sendSSLPostToWx(mapToXml, WechatConfig.getSslcsf()); WechatRefundApiResult
	 * result = (WechatRefundApiResult) XmlUtil.xmlStrToBean(reponseXml,
	 * WechatRefundApiResult.class); return result; }
	 */

	/**
	 * 方法描述：得到请求微信退款请求的参数 创建时间：2017年6月8日 上午11:27:02 作者： xubo
	 *
	 * @param
	 * @return
	 */
	 ...
	 * BigDecimal bdOrderMoney, BigDecimal bdRefundMoney) { Map<Object, Object>
	 * params = new HashMap<Object, Object>(); //微信分配的公众账号ID（企业号corpid即为此appId）
	 * params.put("appid", ResourceUtil.getConfigByName("wx.appId")); //微信支付分配的商户号
	 * params.put("mch_id", ResourceUtil.getConfigByName("wx.mchId"));
	 * //随机字符串，不长于32位。推荐随机数生成算法 params.put("nonce_str",
	 * CharUtil.getRandomString(16)); //商户侧传给微信的订单号 params.put("out_trade_no",
	 * out_trade_no); //商户系统内部的退款单号，商户系统内部唯一，同一退款单号多次请求只退一笔
	 * params.put("out_refund_no", getBundleId()); //订单总金额，单位为分，只能为整数
	 * params.put("total_fee", bdOrderMoney.multiply(new
	 * BigDecimal(100)).intValue()); //退款总金额，订单总金额，单位为分，只能为整数
	 * params.put("refund_fee", bdRefundMoney.multiply(new
	 * BigDecimal(100)).intValue()); //操作员帐号, 默认为商户号 params.put("op_user_id",
	 * ResourceUtil.getConfigByName("wx.mchId")); //签名前必须要参数全部写在前面
	 * params.put("sign", arraySign(params,
	 * ResourceUtil.getConfigByName("wx.paySignKey"))); return params; }
	 */

	/**
	 * ResourceUtil.getConfigByName("wx.refundUrl") 请求微信https
	 **/
	...
	/**
	 * 方法描述：微信查询退款逻辑 创建时间：2017年4月12日 上午11:04:25 作者： xubo
	 *
	 * @param
	 * @return
	 */
	...
	public Map<String, Object> wxRefundquery(String out_trade_no, String out_refund_no) {
		...
		try {
			...
			Map<String, Object> result = XmlUtil.xmlStrToMap(xmlStr);// .xmlStrToBean(xmlStr,
	...
	/**
	 * 方法描述：根据签名加密请求参数 创建时间：2017年6月8日 上午11:28:52 作者： xubo
	 *
	 * @param
	 * @return
	 */
