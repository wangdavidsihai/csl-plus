...
package com.csl.plus.portal.interceptor;
...
public class TokenInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		if (StringUtils.isBlank(authHeader) || "undefined".equals(authHeader.toLowerCase())) {
			log.info(formMapKey(null, "token is null url=" + fullUrl, requestType,
					IpAddressUtil.getIpAddr((HttpServletRequest) request), sbParams.toString(), "null") + ",\"cost\":\""
					+ 0 + "ms\"");
		...
		if (authHeader != null && authHeader.startsWith(this.tokenHead)) {
			...
			if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
				...
				log.info(formMapKey("wu", fullUrl, requestType, IpAddressUtil.getIpAddr((HttpServletRequest) request),
						sbParams.toString(), token) + ",\"cost\":\"" + 0 + "ms\"");
			}
		} else {
			// throw new ApiMallPlusException("请先登录", 401);
		}
	...
	private String formMapKey(Object userId, String mothedName, String requestType, String ip, String params,
			String token) {
		return "\"time\"" + ":\"" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss SSS").format(new Date()) + "\",\"name\""
				+ ":\"" + mothedName + "\",\"uid\":\"" + userId + "\",\"type\":\"" + requestType + "\",\"ip\":\"" + ip
