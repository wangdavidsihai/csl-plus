...
@Slf4j
@RestController
@Api(tags = "/api/ResProductAppAreaController", description = "产品资源应用领域表管理")
@RequestMapping("/api/res/resproductapparea")
...
//    @PreAuthorize("hasAuthority('res:resproductapparea:list')")
    ...
    /**
     ...
     @GetMapping("/info/{id}")
     @PreAuthorize("hasAuthority('res:resproductapparea:info')") public R info(@PathVariable("id") Long id){
...
//    @PreAuthorize("hasAuthority('cms:cmsarticle:read')")
    public Object getResProductAppAreaById(@ApiParam("id") @PathVariable Long id) {
        try {
            if (ValidatorUtils.empty(id)) {
