...
//    @PreAuthorize("hasAuthority('res:resfinanceapparea:list')")
    ...
    /**
     ...
     @GetMapping("/info/{id}")
     @PreAuthorize("hasAuthority('res:resfinanceapparea:info')") public R info(@PathVariable("id") Long id){
    ...
    @GetMapping(value = "/{id}")
    @PreAuthorize("hasAuthority('res:resfinanceapparea:read')")
    public Object getResFinanceAppAreaById(@ApiParam("id") @PathVariable Long id) {
        try {
            if (ValidatorUtils.empty(id)) {
