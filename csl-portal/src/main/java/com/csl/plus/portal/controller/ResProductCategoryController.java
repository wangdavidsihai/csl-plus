...
@Slf4j
@RestController
@Api(tags = "/api/ResProductCategoryController", description = "产品资源类别表管理")
@RequestMapping("/api/res/resproductcategory")
...
//    @PreAuthorize("hasAuthority('res:resproductcategory:list')")
    ...
    /**
     ...
     @GetMapping("/info/{id}")
     @PreAuthorize("hasAuthority('res:resproductcategory:info')") public R info(@PathVariable("id") Long id){
    ...
    @GetMapping(value = "/{id}")
    @PreAuthorize("hasAuthority('res:resproductcategory:read')")
    public Object getResProductCategoryById(@ApiParam("id") @PathVariable Long id) {
        try {
            if (ValidatorUtils.empty(id)) {
