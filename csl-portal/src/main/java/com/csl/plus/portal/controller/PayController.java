...
import com.csl.plus.portal.util.applet.WechatRefundApiResult;
import com.csl.plus.portal.util.applet.WechatUtil;
...
public class PayController extends ApiBaseAction {
	...
	public Object payPrepay(@RequestParam(value = "id", required = false, defaultValue = "0") Long id) {
		...
		try {
			...
			// 订单的商品
			List<OmsOrderItem> orderGoods = orderItemService
					.list(new QueryWrapper<>(new OmsOrderItem()).eq("orderId", id));
			if (null != orderGoods) {
				String body = "超市-";
			...
			log.info("xml:" + xml);
			Map<String, Object> resultUn = XmlUtil
					.xmlStrToMap(WechatUtil.requestOnce(wxAppletProperties.getUniformorder(), xml));
			// 响应报文
	...
	public Object orderQuery(@RequestParam(value = "id", required = false, defaultValue = "0") Long id) {
		...
		if ("SUCCESS".equals(trade_state)) {
			 ...
			 * this.orderQuery(id); } else { return toResponsFail("查询失败,error=" +
			 * trade_state); }
			 */
