...
import com.csl.plus.marking.mapper.SmsRedPacketMapper;
import com.csl.plus.marking.mapper.SmsUserRedPacketMapper;
...
import com.csl.plus.portal.ums.service.IUmsMemberBlanceLogService;
...
import com.csl.plus.ums.entity.UmsMemberBlanceLog;
 ...
 */
@Service
public class SmsRedPacketServiceImpl extends ServiceImpl<SmsRedPacketMapper, SmsRedPacket>
		implements ISmsRedPacketService {
	...
	/**
	 * 剩余红包金额M，剩余人数N，那么：每次抢到金额=随机(0，M/N*2) 保证了每次随机金额的平均值是公平的 假设10人，红包金额100元
	 * 第一人：100/10*2=20，随机范围(0,20)，平均可以抢到10元 第二人：90/9*2=20，随机范围(0,20)，平均可以抢到10元
	 * 第三人：80/8*2=20，随机范围(0,20)，平均可以抢到10元 以此类推，每次随机范围的均值是相等的
	 * 缺点：除了最后一次，任何一次抢到的金额都不会超过人均金额的两倍，并不是任意的随机 ---------------------
	 * 作者：PreciousLife 来源：CSDN
	 ...
	 * <p>
	 * 随机的范围区间是[1，100*
	 * M）。当所有切割点确定以后，子线段的长度也随之确定。这样每个人来抢红包的时候，只需要顺次领取与子线段长度等价的红包金额即可。
	 * --------------------- 作者：PreciousLife 来源：CSDN
	 * 原文：https://blog.csdn.net/q957967519/article/details/84661761
	 * 版权声明：本文为博主原创文章，转载请附上博文链接！
	 *
